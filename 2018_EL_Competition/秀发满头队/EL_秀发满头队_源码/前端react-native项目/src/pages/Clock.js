...
export default class Clock extends Component {
   ...
   constructor(props) {
    ...
    updateTomato=()=>{
      fetch('http://118.25.XX.186/users/'+this.state.username+"/userinfo", {//为避免服务器（未设拦截器）地址泄露，已经隐去一部分地址
            method: 'GET',
            headers: {
    ...
    handleTimesUp(){//记录完成一个周期后的各项数据
      ...
      .then((result) => {
        if(result=="true"){
          ...
          fetch('http://118.25.XX.186/users/'+this.state.username+"/"+this.state.tomato+"/updatetomatoes", {//为避免服务器（未设拦截器）地址泄露，已经隐去一部分地址
             method: 'GET',
             headers: {
                 'Content-Type': 'application/json'
             }
    ...
    upload(){
      this.updateLocation()
      fetch('http://118.25.XX.186/data/create', {//为避免服务器（未设拦截器）地址泄露，已经隐去一部分地址
        method: 'POST',
        headers: {
              'Content-Type': 'application/json'
        },
