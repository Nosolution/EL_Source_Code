...
export default class TimeLine extends Component {
    ...
    componentDidMount() {
        fetch('http://118.25.XX.186/data/', {//为避免服务器（未设拦截器）地址泄露，已经隐去一部分地址
            method: 'GET',
            headers: {
    ...
    updateLocation=()=> {//记录当前位置
        Geolocation.getCurrentPosition()
            .then(data => {
                ...
                fetch('http://118.25.XX.186/users/'+data.latitude+","+data.longitude+"/updatelocation", {//为避免服务器（未设拦截器）地址泄露，已经隐去一部分地址
                method: 'GET',
                headers: {
                  'Content-Type': 'application/json'
                }
    ...
    loadLocation_online=()=>{
        fetch('http://118.25.XX.186/users/'+this.state.name+'/userinfo', {//为避免服务器（未设拦截器）地址泄露，已经隐去一部分地址
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            }
    ...
    joinTask(){
        fetch('http://118.25.XX.186/tasks/join', {//为避免服务器（未设拦截器）地址泄露，已经隐去一部分地址
            method: 'GET',
            headers: {
                  'Content-Type': 'application/json'
            }
    ...
    doLike(item){
        fetch('http://118.25.XX.186/data/'+item._id+'/dolike', {//为避免服务器（未设拦截器）地址泄露，已经隐去一部分地址
        method: 'GET',
        headers: {
              'Content-Type': 'application/json'
        },
    ...
    unLike(item){
        fetch('http://118.25.XX.186/data/'+item._id+'/undolike', {//为避免服务器（未设拦截器）地址泄露，已经隐去一部分地址
        method: 'GET',
        headers: {
              'Content-Type': 'application/json'
        },
